// eslint-disable-file no-lonely-if
// eslint-disable-file no-else-return
// eslint-disable-file jsx-a11y/click-events-have-key-events
// eslint-disable-file jsx-a11y/no-noninteractive-element-interactions
...
const JobImages: React.FC = () => {
    const {job: { job }} = useSelector((state: StoreState) => state);
    ...
    const generateClassName = (i: number):string => {
        if(job.images.length <= 2){
            ...
            if(i === curImgIndex){
                return 'jobGalleryImg curImg';
            } else if (i === curImgIndex - 1 || (i === job.images.length - 1 && curImgIndex === 0)){
                return 'jobGalleryImg moveLeft';
            } else if (i === curImgIndex + 1 || (curImgIndex === job.images.length - 1 && i === 0)){
    ...
    return (
        <div>
            <div 
                className='jobImage' 
                style={{
                    backgroundImage: `url('${job.images && job.images.length && job.images[0].url}')`
                }} 
                onClick={show}
                role="navigation"
            />
